...
def convert_examples_to_features(js, tokenizer, args):  
    ...
    raw_cfg_nodes = js.get('cfg_nodes', [])
    raw_dfg_nodes = js.get('dfg_nodes', [])
    use_word2vec = bool(raw_cfg_nodes or raw_dfg_nodes)
    word2vec_model = get_word2vec_model() if use_word2vec else None
    word2vec_dim = word2vec_model.vector_size if word2vec_model is not None else DEFAULT_WORD2VEC_DIM

    cfg_nodes = [node[1] if isinstance(node, (list, tuple)) and len(node) > 1 else str(node) for node in raw_cfg_nodes]
    if len(cfg_nodes) == 0:  
        cfg_nodes = torch.zeros((1, word2vec_dim), dtype=torch.float32)
    else:  
        cfg_nodes = torch.stack([text_to_embedding(text, word2vec_model) for text in cfg_nodes])  
  
    cfg_edges = js.get('cfg_edges', [])  
    if len(cfg_edges) == 0:  
        cfg_edges = torch.zeros((2, 0), dtype=torch.long)  
    else:  
        cfg_edges = torch.tensor(cfg_edges, dtype=torch.long).t().contiguous()  
  
    dfg_nodes = [node[1] if isinstance(node, (list, tuple)) and len(node) > 1 else str(node) for node in raw_dfg_nodes]
    if len(dfg_nodes) == 0:  
        dfg_nodes = torch.zeros((1, word2vec_dim), dtype=torch.float32)
    else:  
        dfg_nodes = torch.stack([text_to_embedding(text, word2vec_model) for text in dfg_nodes])  
  
    dfg_edges = js.get('dfg_edges', [])  
    if len(dfg_edges) == 0:  
        dfg_edges = torch.zeros((2, 0), dtype=torch.long)  
    else:  
        dfg_edges = torch.tensor(dfg_edges, dtype=torch.long).t().contiguous()  
    return InputFeatures(source_tokens, source_ids, js.get('idx', 0), js['target'], cfg_nodes, cfg_edges, dfg_nodes, dfg_edges)  
...
def resolve_checkpoint_metric(metric_name):
    if metric_name.startswith("eval_"):
        return metric_name
    return f"eval_{metric_name}"


def build_eval_result(eval_loss, labels, scores, threshold):
    preds = (scores >= threshold).astype(int)
    classification = compute_binary_classification_metrics(labels.tolist(), preds.tolist())
    result = {
        "eval_loss": float(eval_loss),
        "eval_threshold": float(threshold),
        "eval_acc": round(classification["accuracy"], 4),
        "eval_accuracy": round(classification["accuracy"], 4),
    }
    for key, value in classification.items():
        result_key = f"eval_{key}"
        result[result_key] = round(value, 4) if isinstance(value, float) else int(value)
    return result, preds
...
def train(args, train_dataset, model, tokenizer):  
    ...
    checkpoint_metric = resolve_checkpoint_metric(args.checkpoint_metric)
    model.zero_grad()  
                ...
                if args.save_steps > 0 and global_step % args.save_steps == 0:
                    if args.evaluate_during_training:  
                        ...
                        if args.local_rank in [-1, 0]:  
                            ...
                            checkpoint_score = results.get(checkpoint_metric)
                            if checkpoint_score is None:
                                raise KeyError(f"Checkpoint metric '{checkpoint_metric}' not found in evaluation results.")
                            if checkpoint_score > best_checkpoint_score:
                                best_checkpoint_score = checkpoint_score
                                logger.info("  " + "*" * 20)
                                logger.info("  Best %s:%s", checkpoint_metric, round(best_checkpoint_score, 4))
                                logger.info("  " + "*" * 20)
...
def evaluate(args, model, tokenizer, eval_when_training=False):  
    ...
    if args.local_rank in [-1, 0]:  
        ...
        scores = logits_np[:, 0]
        result, _ = build_eval_result(float(eval_loss), labels_np, scores, args.prediction_threshold)
...
def test(args, model, tokenizer):  
    ...
    for batch in tqdm(eval_dataloader, total=len(eval_dataloader)):  
        ...
        dfg_batch = build_dfg_data_list(dfg_nodes, dfg_edges).to(args.device)  
        with torch.no_grad():  
            logit = model(inputs, labels=None, cfg=cfg_batch, dfg=dfg_batch)
            logits_list.append(logit)  
            labels_list.append(label)  
    ...
    preds = (all_logits[:, 0] >= args.prediction_threshold).astype(int)
    if args.local_rank in [-1, 0]:  
...
def main():  
    ...
    parser.add_argument('--prediction_threshold', type=float, default=0.5,
                        help='Probability threshold used to convert scores into binary predictions.')
    parser.add_argument('--checkpoint_metric', type=str, default='eval_f1',
                        help='Evaluation metric key used to select the best checkpoint.')
